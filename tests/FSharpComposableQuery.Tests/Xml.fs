...
module Xml =
    ...
    type sql = SqlDataProvider<
            ...
            >
    type DataEntity = sql.dataContext.``main.DataEntity``
    type Text       = sql.dataContext.``main.DataEntity``
    type Attribute  =  sql.dataContext.``main.AttributeEntity``
...
//        data.Context.SubmitChanges()
    ...
    let [<Literal>] xmlPath = __SOURCE_DIRECTORY__ + "/data/movies.xml"
    ...
    [<Test>]
    let test01() =
        xp0 |> xpath 0 <@ data.AsQueryable() @>  |> Utils.Run

    [<Test>]
    let test02() =
        xp1 |> xpath 0 <@ data.AsQueryable() @> |> Utils.Run

    [<Test>]
    let test03() =
        xp2 |> xpath 0 <@ data.AsQueryable() @> |> Utils.Run

    [<Test>]
    let test04() =
        xp3 |> xpath 0 <@ data.AsQueryable() @> |> Utils.Run